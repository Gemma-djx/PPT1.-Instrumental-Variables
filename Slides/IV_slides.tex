...
\begin{frame}
\begin{center}
\includegraphics[width=\textwidth, height=0.85\textheight, keepaspectratio]{./lecture_includes/schedule.png}
\end{center}
\end{frame}
...
\begin{itemize}\itemsep0em
  ...
  \begin{itemize}\itemsep0em
    \item E.g. a ``structural'' model of supply/demand, or a potential outcome model linking schooling to earnings
  ...
  \begin{itemize}\itemsep0em
    ...
    \item Use estimator distributions to learn about estimands (inference)
  \end{itemize}
\end{itemize}
}
\end{frame}
...
\begin{frame}{The Lay of the Land}
  ...
  \vspace{4mm}
  {\small Separating out the very different tasks of identification and inference can help make our lives easier\smallskip\pause{}
\begin{itemize}
...
\begin{frame}{Angrist (1990): IV Specification}
{\small
\textbf{Setting:} Vietnam-era draft eligibility as IV for military service effects on earnings.\medskip

\textbf{First Stage} (eligibility $\to$ military service):
\vspace{-0.3cm}
\begin{align*}
D_i &= \mu + \pi \cdot Z_i + \eta_i
\end{align*}
\vspace{-0.5cm}
where $Z_i \in \{0,1\}$ = draft eligibility (lottery), $D_i \in \{0,1\}$ = actual military service.\medskip

\textbf{Reduced Form} (eligibility $\to$ earnings):
\vspace{-0.3cm}
\begin{align*}
Y_i &= \kappa + \rho \cdot Z_i + \nu_i
\end{align*}
\vspace{-0.5cm}
where $Y_i$ = annual earnings (measured 1971, 1981, 1984).\medskip

\textbf{IV Estimate} (Wald):
\vspace{-0.3cm}
\begin{align*}
\hat{\beta}^{IV} = \frac{\hat{\rho}}{\hat{\pi}} = \frac{E[Y_i \mid Z_i=1]-E[Y_i \mid Z_i=0]}{E[D_i \mid Z_i=1]-E[D_i \mid Z_i=0]}
\end{align*}
}
{\scriptsize \textcolor{asher}{\href{https://www.jstor.org/stable/2006669}{Angrist (1990), ``Lifetime Earnings and the Vietnam Era Draft Lottery,'' \emph{AER} 80(3)}}}
\end{frame}
...
\begin{center}
\includegraphics[width=0.85\textwidth, keepaspectratio]{./lecture_includes/ak_table.png}
\end{center}

\end{frame}

\begin{frame}{Angrist-Krueger (1991): IV Specification}
{\small
\textbf{Setting:} Quarter of birth (QOB) as IV for schooling effects on earnings.\smallskip

\textbf{Instrument:} $Z_{iq} \in \{0,1\}$ = born in quarter $q$. Compulsory attendance laws force Q1 students to stay in school longer before dropping out.\smallskip

\textbf{First Stage} (QOB $\to$ years of schooling):
\vspace{-0.4cm}
\begin{align*}
S_i = \mu + \sum_{q=2}^{4} \pi_q \cdot Z_{iq} + W_i'\psi + \eta_i
\end{align*}
\vspace{-0.6cm}
\textbf{Reduced Form} (QOB $\to$ log weekly wages):
\vspace{-0.4cm}
\begin{align*}
Y_i = \kappa + \sum_{q=2}^{4} \rho_q \cdot Z_{iq} + W_i'\phi + \nu_i
\end{align*}
\vspace{-0.6cm}
\textbf{2SLS:} $\hat{\beta}^{2SLS}$ from overidentified system (up to 30 QOB$\times$YOB instruments)
}\\[0.1em]
{\scriptsize \textcolor{asher}{\href{https://academic.oup.com/qje/article/106/4/979/1917545}{Angrist \& Krueger (1991), ``Does Compulsory School Attendance Affect Schooling and Earnings?'' \emph{QJE} 106(4)}}}
\end{frame}
...
\begin{center}
\includegraphics[width=0.75\textwidth, keepaspectratio]{./lecture_includes/strongpi.png}
\end{center}
...
\begin{center}
\includegraphics[scale=0.32]{./lecture_includes/medpi.png}
\end{center}
...
\begin{center}
\includegraphics[scale=0.32]{./lecture_includes/weakpi.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.75\textwidth, keepaspectratio]{./lecture_includes/fewz.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.75\textwidth, keepaspectratio]{./lecture_includes/somez.png}
\end{center}
...
\begin{center}
\includegraphics[width=0.75\textwidth, keepaspectratio]{./lecture_includes/manyz.png}
\end{center}
...
\vspace{-0.2cm}
{\small Aim for few instruments, and check your F's after every \emph{ivreg}\smallskip
\begin{itemize}
...
\item See Lee et al. (2020) and Keane and Neal (2022) for additional subtleties
\end{itemize}
\medskip\pause{}
...
\subsection{Stata Implementation}

\begin{frame}{Key Stata Packages for IV Estimation}
\vspace{-0.2cm}
{\small Four essential packages to install before running any IV regression:\medskip

\begin{tabular}{lll}
\hline
\textbf{Package} & \textbf{Purpose} & \textbf{Install Command} \\
\hline
\code{ivreg2} & Standard 2SLS/LIML & \code{ssc install ivreg2} \\
\code{ivreghdfe} & 2SLS with HD fixed effects & \code{ssc install ivreghdfe} \\
\code{ranktest} & Rank condition tests & \code{ssc install ranktest} \\
\code{weakivtest} & Robust weak IV test & \code{ssc install weakivtest} \\
\code{ssaggregate} & Shift-share aggregation & \code{ssc install ssaggregate} \\
\code{boottest} & Wild score bootstrap & \code{ssc install boottest} \\
\hline
\end{tabular}\medskip

\textbf{Dependencies:} \code{ivreghdfe} requires \code{hdfe} and \code{ftools}:\smallskip

\hspace{1em}\code{ssc install ftools}

\hspace{1em}\code{ssc install hdfe}

\hspace{1em}\code{ssc install ivreghdfe}
}
\end{frame}

\begin{frame}{Stata IV Workflow: A Quick Reference}
\vspace{-0.2cm}
{\small
\textbf{Step 1 --- Basic 2SLS} (\code{ivreg2}):\smallskip

\hspace{1em}\code{ivreg2 y \sun{covs} (\electricViolet{endo} = \pictonBlue{inst}), robust first}

\vspace{-0.2cm}
Options: \code{robust} (HC1 SE), \code{first} (print first stage), \code{savefirst}\medskip

\textbf{Step 2 --- With High-Dimensional FE} (\code{ivreghdfe}):\smallskip

\hspace{1em}\code{ivreghdfe y \sun{covs} (\electricViolet{endo} = \pictonBlue{inst}), robust absorb(\textit{fes})}\medskip

\textbf{Step 3 --- Weak Instrument Test} (after \code{ivreg2}):\smallskip

\hspace{1em}\code{weakivtest}\quad (Montiel Olea--Pflueger effective F-statistic)\medskip

\textbf{Step 4 --- Shift-Share IV} (\code{ssaggregate}):\smallskip

\hspace{1em}\code{ssaggregate y endo (inst = shock), n(ind) s(share)}

\hspace{1em}\code{ivreg2 y (endo = inst), robust}
}
\end{frame}
...
\begin{center}
	\includegraphics[width=0.9\textwidth, keepaspectratio]{./lecture_includes/duflo_2001.png}
\end{center}
{\small Here $Z_i=$ growing up in a region with intensive school building}
\end{frame}

\begin{frame}{Duflo (2001): IV Specification}
{\small
\textbf{Setting:} Indonesia's 1973--79 school construction program (INPRES): 61,000+ schools built, intensity varied by region. Instrument for schooling effects on wages.\medskip

\textbf{DiD First Stage} (program intensity $\to$ schooling):
\vspace{-0.3cm}
\begin{align*}
S_{ic} = \alpha_c + \lambda_t + \pi \cdot \underbrace{(P_c \times \mathbf{1}[\text{young}_{it}])}_{Z_{ic}} + W_{ic}'\psi + \eta_{ic}
\end{align*}
\vspace{-0.5cm}
where $P_c$ = schools built per 1000 children in region $c$, ``young'' = cohort exposed to program.\medskip

\textbf{DiD Reduced Form} (program $\to$ log wages):
\vspace{-0.3cm}
\begin{align*}
Y_{ic} = \kappa_c + \tau_t + \rho \cdot (P_c \times \mathbf{1}[\text{young}_{ic}]) + W_{ic}'\phi + \nu_{ic}
\end{align*}
\vspace{-0.5cm}
\textbf{IV Estimate:} $\hat{\beta}^{IV} = \hat{\rho}/\hat{\pi}$ = return to one additional year of schooling
}
{\scriptsize \textcolor{asher}{\href{https://www.aeaweb.org/articles?id=10.1257/aer.91.4.795}{Duflo (2001), ``Schooling and Labor Market Consequences of School Construction in Indonesia,'' \emph{AER} 91(4)}}}
\end{frame}
...
\begin{frame}{Autor, Dorn, and Hanson (2014): IV Specification}
{\small
\textbf{Setting:} Chinese import competition effects on US local labor markets, 1990--2007.\medskip

\textbf{Shift-Share Instrument:}
\vspace{-0.3cm}
\begin{align*}
Z_{it} = \sum_{k} \underbrace{s_{ik,t-1}}_{\text{emp. share}} \cdot \underbrace{g_{kt}^{\text{other}}}_{\text{import growth}} = \sum_k \frac{L_{ikt-1}}{L_{it-1}} \cdot \frac{\Delta M_{kt}^{\text{other}}}{L_{kt-1}}
\end{align*}
\vspace{-0.5cm}
\textbf{First Stage} (instrument $\to$ actual import growth in CZ $i$):
\vspace{-0.3cm}
\begin{align*}
\Delta IPW_{it} = \alpha_t + \pi \cdot Z_{it} + W_{it}'\psi + \eta_{it}
\end{align*}
\vspace{-0.5cm}
\textbf{Second Stage} (import growth $\to$ manufacturing employment change):
\vspace{-0.3cm}
\begin{align*}
\Delta L_{it}^{\text{mfg}} / L_{it-1} = \beta \cdot \widehat{\Delta IPW}_{it} + W_{it}'\phi + \nu_{it}
\end{align*}
}
{\scriptsize \textcolor{asher}{\href{https://www.aeaweb.org/articles?id=10.1257/aer.104.6.1553}{Autor, Dorn \& Hanson (2014), ``The China Syndrome,'' \emph{AER} 104(6)}}}
\end{frame}
...
\vspace{-0.3cm}
	\begin{center}
		...
		\includegraphics[trim={1cm 0.5cm 0.5cm 1cm},clip,width=0.85\textwidth, keepaspectratio]{lecture_includes/Lines_actual_planned_nocolor.png}
	\end{center}
\end{frame}
...
\vspace{-0.3cm}
	\begin{center}
		...
		\includegraphics[trim={1cm 0.5cm 0.5cm 1cm},clip,width=0.85\textwidth, keepaspectratio]{lecture_includes/Line_panel2016.png}
	\end{center}
\end{frame}
...
\vspace{-0.3cm}
	\begin{center}
		...
		\includegraphics[trim={1cm 0.5cm 0.5cm 1cm},clip,width=0.85\textwidth, keepaspectratio]{lecture_includes/Sim_Line_nlink2016.png}
	\end{center}
\end{frame}
...
\vspace{-0.3cm}
	\begin{center}
		...
		\includegraphics[trim={1cm 0.5cm 0.5cm 1cm},clip,width=0.85\textwidth, keepaspectratio]{lecture_includes/DateExpected2016.png}
	\end{center}
\end{frame}
...
Recentered MA growth, $Z_i$
	...
	\includegraphics[trim={1cm 0.5cm 0cm 1cm},clip,width=0.85\textwidth, keepaspectratio]{lecture_includes/NlinkRecentered2016.png}
	\end{center}
\end{frame}
...
\begin{frame}{Illustration: ACA Medicaid Expansion}
	...
	\begin{center}
	\includegraphics[width=0.95\textwidth, keepaspectratio]{lecture_includes/aca_ss.png}
	\end{center}
...
\begin{frame}{Four Key Papers: Quick Reference \& Links}
\vspace{-0.2cm}
{\small The four main empirical examples covered in these slides:\bigskip

\begin{enumerate}\itemsep0.6em
  \item \textbf{Angrist (1990)} --- Vietnam Draft Lottery IV \\
  {\footnotesize Returns to military service. \emph{AER} 80(3). \href{https://www.jstor.org/stable/2006669}{\texttt{jstor.org/stable/2006669}}}

  \item \textbf{Angrist \& Krueger (1991)} --- Quarter of Birth IV \\
  {\footnotesize Returns to schooling via compulsory attendance laws. \emph{QJE} 106(4). \href{https://academic.oup.com/qje/article/106/4/979/1917545}{\texttt{doi:10.2307/2937954}}}

  \item \textbf{Duflo (2001)} --- Indonesia School Construction IV (DiD) \\
  {\footnotesize Schooling and wages; INPRES program as instrument. \emph{AER} 91(4). \href{https://www.aeaweb.org/articles?id=10.1257/aer.91.4.795}{\texttt{aeaweb.org/10.1257/aer.91.4.795}}}

  \item \textbf{Autor, Dorn \& Hanson (2014)} --- China Shock Shift-Share \\
  {\footnotesize Import competition and US local labor markets. \emph{AER} 104(6). \href{https://www.aeaweb.org/articles?id=10.1257/aer.104.6.1553}{\texttt{aeaweb.org/10.1257/aer.104.6.1553}}}
\end{enumerate}
}
\end{frame}
